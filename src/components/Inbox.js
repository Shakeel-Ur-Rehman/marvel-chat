...
import  {Change_Active_Inbox} from '../lib/redux/Actions/conversationActions'
 ...
 function Inbox(props) {
    ...
    return (  
        ...
        <Container style={{height:"500px"}} fluid={true}>
        
        {
            props.loader?
            <img style={{width:"35px",height:"35px",margin :" 20px 25px  "}} src="images/45.gif"/>
            :result.map(value=>
                ...
                <div style={{float:"left",width:"77%",marginTop:"8px"}}>
                    <h5 style={{marginBottom:"unset",fontSize:"18px",fontWeight:700}}>{value.topic}</h5>
                    <p style={{fontSize:"14px",marginBottom:"unset"}}>{value.description}</p>
                </div>
            </div>
            )
        }
